...
pub async fn initialize_schema(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    // Create table for storing table definitions
    ...
    // Create table for storing column definitions
    ...
    query.add_column(ColumnBuilder::new(
        ColumnIdentifier::Hardcoded("data_type"),
        RawDataType::Text,
    ));
    ...
    // Create default schema for notes
