...
fn vs_main(
    ...
    var position: vec4<f32> = vec4<f32>((((pos.xy + t.data1.zw) * prim.scale) * invert_y), 0.0, 1.0);
    position.x = (position.x / global.size.x) - 1;
    position.y = (position.y / global.size.y) + 1;
